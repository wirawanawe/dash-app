...
const DEFAULT_POLL_INTERVAL_MS = Math.max(
  3000,
  parseInt(process.env.VISITS_SSE_POLL_INTERVAL_MS || '10000', 10)
);
const DEFAULT_HEARTBEAT_MS = Math.max(
  5000,
  parseInt(process.env.VISITS_SSE_HEARTBEAT_MS || '20000', 10)
);
...
async function fetchLatestSnapshot() {
  try {
      ...
      `SELECT 
         ...
         UNIX_TIMESTAMP(
           GREATEST(
             ...
             IFNULL(MAX(synced_at), '1970-01-01 00:00:00'),
    ...
    return {
      version: visitRow?.version ?? 0,
      total: visitRow?.total ?? 0,
      syncLog: logRow
        ? {
            id: logRow.id,
