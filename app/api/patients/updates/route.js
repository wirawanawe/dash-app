...
const DEFAULT_POLL_INTERVAL_MS = Math.max(
  3000,
  parseInt(process.env.PATIENTS_SSE_POLL_INTERVAL_MS || '10000', 10)
);
const DEFAULT_HEARTBEAT_MS = Math.max(
  5000,
  parseInt(process.env.PATIENTS_SSE_HEARTBEAT_MS || '20000', 10)
);
...
async function fetchLatestSnapshot() {
  try {
      ...
      `SELECT 
         ...
         UNIX_TIMESTAMP(
           GREATEST(
             ...
             IFNULL(MAX(external_updated_at), '1970-01-01 00:00:00')
           )
         ) AS version
       FROM patients`
    );
    ...
    return {
      version: patientRow?.version ?? 0,
      total: patientRow?.total ?? 0,
      syncLog: logRow
        ? {
            id: logRow.id,
...
export async function GET() {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      controller.enqueue(
        formatSseEvent('patients:bootstrap', {
      ...
      const poll = async () => {
        ...
        try {
          ...
          if (hasNewVersion || hasCountChanged || hasNewSync || syncStatusChanged || (isSyncInProgress && !previousSyncStatus)) {
            ...
            controller.enqueue(
              formatSseEvent('patients:refresh', {
                version: previousVersion,
                total: previousTotal,
