...
async function syncFull(options = {}) {
  ...
  const config = {
    maxRecords: options.maxRecords || 2000,        // Reduced from 5000 to 2000
    recordsPerPage: options.recordsPerPage || 200, // Reduced from 500 to 200
    concurrentPages: options.concurrentPages || 1, // Reduced from 3 to 1 (sequential)
    batchSize: options.batchSize || 30,            // Reduced from 100 to 30
    delayBetweenBatches: options.delayBetweenBatches || 3000, // Increased from 500ms to 3s
    delayBetweenPages: options.delayBetweenPages || 2000,     // Add 2s delay between pages
  };
