...
export async function verifyJwtToken(token) {
  if (!token) {
    console.log("verifyJwtToken: No token provided");
    return null;
  }
  ...
  try {
    ...
    if (!process.env.JWT_SECRET) {
      console.error(
        "verifyJwtToken: JWT_SECRET is not defined in environment variables"
      );
      return null;
    }

    console.log("verifyJwtToken: Attempting to verify token");
    const secretKey = new TextEncoder().encode(process.env.JWT_SECRET);
    const { payload } = await jwtVerify(token, secretKey);
    console.log("verifyJwtToken: Token verified successfully");
    return payload;
  } catch (error) {
    console.error("verifyJwtToken: Token verification failed:", error.message);
    return null;
  }
}
