-- Update Existing Mission Progress Based on Tracking Data (Fixed)
...
-- Update Calorie Missions based on meal_logging data
UPDATE user_missions um
JOIN missions m ON um.mission_id = m.id
JOIN (
    SELECT 
...
) ml ON um.user_id = ml.user_id AND um.mission_date = ml.meal_date
SET 
    um.current_value = ml.total_calories,
    um.progress = CASE 
        WHEN m.target_value > 0 THEN LEAST((ml.total_calories / m.target_value) * 100, 100)
        ELSE 0 
    END,
    um.status = CASE 
...
-- Update Meal Count Missions based on meal_logging data
UPDATE user_missions um
JOIN missions m ON um.mission_id = m.id
JOIN (
    SELECT 
...
SET 
    ...
    um.progress = CASE 
        WHEN m.target_value > 0 THEN LEAST((ml.meal_count / m.target_value) * 100, 100)
        ELSE 0 
    END,
    um.status = CASE 
